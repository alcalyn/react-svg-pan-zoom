...
var common_typeof = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function (obj) { return typeof obj; } : function (obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; };

var common_extends = Object.assign || function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; };
 ...
 * @returns {Object}
 */
function common_getDefaultValue(viewerWidth, viewerHeight, SVGWidth, SVGHeight, scaleFactorMin, scaleFactorMax) {
  return common_set({}, common_extends({}, Object(transformation_matrix_min["identity"])(), {
    version: 2,
    mode: MODE_IDLE,
    focus: false,
 ...
 * @returns {Object}
 */
function common_set(value, change) {
 ...
 * @param value
 */
function common_isValueValid(value) {
  return value !== null && (typeof value === 'undefined' ? 'undefined' : common_typeof(value)) === 'object' && value.hasOwnProperty('version');
}
 ...
 */
function common_getSVGPoint(value, viewerX, viewerY) {
 ...
 */
function common_decompose(value) {
  var matrix = Object(transformation_matrix_min["fromObject"])(value);
 ...
 * @returns {Object}
 */
function common_setFocus(value, focus) {
  return common_set(value, { focus: focus });
}
 ...
 * @returns {Object}
 */
function common_setViewerSize(value, viewerWidth, viewerHeight) {
  return common_set(value, { viewerWidth: viewerWidth, viewerHeight: viewerHeight });
}
 ...
 * @returns {Object}
 */
function common_setSVGSize(value, SVGWidth, SVGHeight) {
  return common_set(value, { SVGWidth: SVGWidth, SVGHeight: SVGHeight });
}
 ...
 * @returns {Object}
 */
function common_setZoomLevels(value, scaleFactorMin, scaleFactorMax) {
  return common_set(value, { scaleFactorMin: scaleFactorMin, scaleFactorMax: scaleFactorMax });
}
...
function common_setPointOnViewerCenter(value, SVGPointX, SVGPointY, zoomLevel) {
  ...
  return common_set(value, common_extends({
    mode: MODE_IDLE
  }, matrix));
}
 ...
 * @returns {Object}
 */
function common_resetMode(value) {
  return common_set(value, {
    mode: MODE_IDLE,
    startX: null,
    startY: null,
    endX: null,
...
var viewer_event_createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }();

function viewer_event_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }



var viewer_event_ViewerEvent = function () {
  function ViewerEvent(originalEvent, value, SVGViewer) {
    viewer_event_classCallCheck(this, ViewerEvent);
  ...
  viewer_event_createClass(ViewerEvent, [{
    key: 'preventDefault',
    ...
    key: 'scaleFactor',
    get: function get() {
      this._cacheDecomposedValue = this._cacheDecomposedValue || common_decompose(this.value);
...
function viewer_mouse_event_possibleConstructorReturn(self, call) { if (!self) { throw new ReferenceError("this hasn't been initialised - super() hasn't been called"); } return call && (typeof call === "object" || typeof call === "function") ? call : self; }

function viewer_mouse_event_inherits(subClass, superClass) { if (typeof superClass !== "function" && superClass !== null) { throw new TypeError("Super expression must either be null or a function, not " + typeof superClass); } subClass.prototype = Object.create(superClass && superClass.prototype, { constructor: { value: subClass, enumerable: false, writable: true, configurable: true } }); if (superClass) Object.setPrototypeOf ? Object.setPrototypeOf(subClass, superClass) : subClass.__proto__ = superClass; }




var viewer_mouse_event_ViewerMouseEvent = function (_ViewerEvent) {
  viewer_mouse_event_inherits(ViewerMouseEvent, _ViewerEvent);

  function ViewerMouseEvent() {
    viewer_mouse_event_classCallCheck(this, ViewerMouseEvent);

    return viewer_mouse_event_possibleConstructorReturn(this, (ViewerMouseEvent.__proto__ || Object.getPrototypeOf(ViewerMouseEvent)).apply(this, arguments));
  }
  ...
  viewer_mouse_event_createClass(ViewerMouseEvent, [{
    ...
    get: function get() {
      if (!this._cachePoint) {
        ...
        this._cachePoint = common_getSVGPoint(value, x, y);
      }
  ...
  viewer_touch_event_createClass(ViewerTouchEvent, [{
    ...
    value: function touchesToPoints(touches, SVGViewer, value) {
      ...
      for (var i = 0; i < touches.length; i++) {
        ...
        var point = common_getSVGPoint(value, x, y);
...
function pan_pan(value, SVGDeltaX, SVGDeltaY) {
  ...
  if (panLimit) {
    var _applyToPoints = Object(transformation_matrix_min["applyToPoints"])(matrix, [{ x: panLimit, y: panLimit }, { x: value.SVGWidth - panLimit, y: value.SVGHeight - panLimit }]),
        _applyToPoints2 = pan_slicedToArray(_applyToPoints, 2),
  ...
  return common_set(value, pan_extends({
    mode: MODE_IDLE
  }, matrix), ACTION_PAN);
}

function pan_startPanning(value, viewerX, viewerY) {
  return common_set(value, {
    mode: MODE_PANNING,
    startX: viewerX,
    startY: viewerY,
...
function pan_updatePanning(value, viewerX, viewerY, panLimit) {
  ...
  var start = common_getSVGPoint(value, endX, endY);
  var end = common_getSVGPoint(value, viewerX, viewerY);

  var deltaX = end.x - start.x;
  var deltaY = end.y - start.y;

  var nextValue = pan_pan(value, deltaX, deltaY, panLimit);
  return common_set(nextValue, {
    mode: MODE_PANNING,
    endX: viewerX,
    endY: viewerY
  }, ACTION_PAN);
...
function pan_stopPanning(value) {
  return common_set(value, {
    mode: MODE_IDLE,
    startX: null,
    startY: null,
...
function pan_autoPanIfNeeded(value, viewerX, viewerY) {
  var deltaX = 0;
  var deltaY = 0;
...
function zoom_lessThanScaleFactorMin(value, scaleFactor) {
  return value.scaleFactorMin && value.d * scaleFactor <= value.scaleFactorMin;
}

function zoom_moreThanScaleFactorMax(value, scaleFactor) {
  return value.scaleFactorMax && value.d * scaleFactor >= value.scaleFactorMax;
}

function zoom_isZoomLevelGoingOutOfBounds(value, scaleFactor) {
  return zoom_lessThanScaleFactorMin(value, scaleFactor) && scaleFactor < 1 || zoom_moreThanScaleFactorMax(value, scaleFactor) && scaleFactor > 1;
}

function zoom_limitZoomLevel(value, matrix) {
...
function zoom_zoom(value, SVGPointX, SVGPointY, scaleFactor) {
  if (zoom_isZoomLevelGoingOutOfBounds(value, scaleFactor)) {
  ...
  return common_set(value, zoom_extends({
    mode: MODE_IDLE
  }, zoom_limitZoomLevel(value, matrix), {
    startX: null,
    startY: null,
    endX: null,
    endY: null
...
function zoom_fitSelection(value, selectionSVGPointX, selectionSVGPointY, selectionWidth, selectionHeight) {
  ...
  if (zoom_isZoomLevelGoingOutOfBounds(value, scaleLevel / value.d)) {
...
function zoom_zoomOnViewerCenter(value, scaleFactor) {
  ...
  var SVGPoint = common_getSVGPoint(value, viewerWidth / 2, viewerHeight / 2);
  return zoom_zoom(value, SVGPoint.x, SVGPoint.y, scaleFactor);
}

function zoom_startZooming(value, viewerX, viewerY) {
  return common_set(value, {
    mode: MODE_ZOOMING,
    startX: viewerX,
    startY: viewerY,
...
function zoom_updateZooming(value, viewerX, viewerY) {
...
function zoom_stopZooming(value, viewerX, viewerY, scaleFactor, props) {
  ...
  var start = common_getSVGPoint(value, startX, startY);
  var end = common_getSVGPoint(value, endX, endY);
  ...
  if (Math.abs(startX - endX) > 7 && Math.abs(startY - endY) > 7) {
    ...
    var SVGPoint = common_getSVGPoint(value, viewerX, viewerY);
...
function interactions_onMouseDown(event, ViewerDOM, tool, value, props) {
  ...
  switch (tool) {
    case TOOL_ZOOM_OUT:
      var SVGPoint = common_getSVGPoint(value, x, y);
      nextValue = zoom_zoom(value, SVGPoint.x, SVGPoint.y, 1 / props.scaleFactor, props);
      break;

    case TOOL_ZOOM_IN:
      nextValue = zoom_startZooming(value, x, y);
      break;

    case TOOL_AUTO:
    case TOOL_PAN:
      nextValue = pan_startPanning(value, x, y);
      break;
...
function interactions_onMouseMove(event, ViewerDOM, tool, value, props) {
  ...
  switch (tool) {
    case TOOL_ZOOM_IN:
      if (value.mode === MODE_ZOOMING) nextValue = forceExit ? zoom_stopZooming(value, x, y, props.scaleFactor, props) : zoom_updateZooming(value, x, y);
      break;

    case TOOL_AUTO:
    case TOOL_PAN:
      if (value.mode === MODE_PANNING) nextValue = forceExit ? pan_stopPanning(value) : pan_updatePanning(value, x, y, props.preventPanOutside ? 20 : undefined);
      break;
...
function interactions_onMouseUp(event, ViewerDOM, tool, value, props) {
  ...
  switch (tool) {
    case TOOL_ZOOM_OUT:
      if (value.mode === MODE_ZOOMING) nextValue = zoom_stopZooming(value, x, y, 1 / props.scaleFactor, props);
      break;

    case TOOL_ZOOM_IN:
      if (value.mode === MODE_ZOOMING) nextValue = zoom_stopZooming(value, x, y, props.scaleFactor, props);
      break;

    case TOOL_AUTO:
    case TOOL_PAN:
      if (value.mode === MODE_PANNING) nextValue = pan_stopPanning(value, x, y);
      break;
...
function interactions_onMouseEnterOrLeave(event, ViewerDOM, tool, value, props) {
  var coords = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : null;

  var nextValue = common_setFocus(value, event.type === 'mouseenter');

  event.preventDefault();
  return nextValue;
}

function interactions_onInterval(event, ViewerDOM, tool, value, props) {
  var coords = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : null;
  var x = coords.x,
      y = coords.y;
  ...
  return pan_autoPanIfNeeded(value, x, y);
}
...
function interactions_touch_hasPinchPointDistance(value) {
  return typeof value.pinchPointDistance === 'number';
}

function interactions_touch_onMultiTouch(event, ViewerDOM, tool, value, props) {
  var _ViewerDOM$getBoundin = ViewerDOM.getBoundingClientRect(),
      left = _ViewerDOM$getBoundin.left,
      top = _ViewerDOM$getBoundin.top;
  ...
  var previousPointDistance = interactions_touch_hasPinchPointDistance(value) ? value.pinchPointDistance : pinchPointDistance;
  var svgPoint = common_getSVGPoint(value, (x1 + x2) / 2, (y1 + y2) / 2);
  var distanceFactor = pinchPointDistance / previousPointDistance;

  if (zoom_isZoomLevelGoingOutOfBounds(value, distanceFactor)) {
    // Do not change translation and scale of value
    return value;
  }
  ...
  return common_set(value, common_set(interactions_touch_extends({
    mode: MODE_ZOOMING
  }, zoom_limitZoomLevel(value, matrix), {
    startX: null,
    startY: null,
...
function interactions_touch_isMultiTouch(event, props) {
  return props.detectPinchGesture && event.touches.length > 1;
}

function interactions_touch_shouldResetPinchPointDistance(event, value, props) {
  return props.detectPinchGesture && interactions_touch_hasPinchPointDistance(value) && event.touches.length < 2;
}

function interactions_touch_getTouchPosition(touch, ViewerDOM) {
  var _ViewerDOM$getBoundin2 = ViewerDOM.getBoundingClientRect(),
      left = _ViewerDOM$getBoundin2.left,
      top = _ViewerDOM$getBoundin2.top;
...
function interactions_touch_getNextValue(event, ViewerDOM, tool, value, props, nextValueFn) {
  var nextValue = event.touches.length === 0 ? common_set(value, { mode: value.prePinchMode ? MODE_IDLE : value.mode, prePinchMode: null }) : value;
  var touch = event.touches.length > 0 ? event.touches[0] : event.changedTouches[0];
  var touchPosition = interactions_touch_getTouchPosition(touch, ViewerDOM);
...
function interactions_touch_onTouchStart(event, ViewerDOM, tool, value, props) {
  if (interactions_touch_isMultiTouch(event, props)) {
    return interactions_touch_onMultiTouch(event, ViewerDOM, tool, value, props);
  }

  if (event.touches.length !== 1) {
    if ([MODE_PANNING, MODE_ZOOMING].indexOf(value.mode) >= 0) {
      return common_resetMode(value);
    } else if ([MODE_IDLE].indexOf(value.mode) >= 0) {
      return value;
    }
  }

  return interactions_touch_getNextValue(event, ViewerDOM, tool, value, props, interactions_onMouseDown);
}
...
function interactions_touch_onTouchMove(event, ViewerDOM, tool, value, props) {
  ...
  return interactions_touch_getNextValue(event, ViewerDOM, tool, value, props, interactions_onMouseMove);
}
...
function interactions_touch_onTouchEnd(event, ViewerDOM, tool, value, props) {
  ...
  var nextValue = interactions_touch_shouldResetPinchPointDistance(event, value, props) ? common_set(value, { pinchPointDistance: null }) : value;
...
var RandomUID_uid = 1;
var RandomUID_nextUID = function nextUID() {
  return "uid" + RandomUID_uid++;
};
...
function RandomUID_RandomUID(WrappedComponent) {
  var RandomUID = function (_React$Component) {
    ...
    function RandomUID(props) {
      ...
      _this.state = { uid: RandomUID_nextUID() };
      return _this;
    }
...
var border_gradient_prefixID = 'react-svg-pan-zoom_border_gradient';

function border_gradient_BorderGradient(_ref) {
  ...
  var gradientID = border_gradient_prefixID + '_gradient_' + _uid;
  var maskID = border_gradient_prefixID + '_mask_' + _uid;
...
border_gradient_BorderGradient.propTypes = {
...
/* harmony default export */ var border_gradient = (RandomUID_RandomUID(border_gradient_BorderGradient));
// CONCATENATED MODULE: ./src/ui/selection.jsx




function selection_Selection(_ref) {
...
function icon_cursor_IconCursor() {
  return external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(
    "svg",
...
function icon_zoom_in_IconZoomIn() {
  return external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(
    "svg",
    { width: 24, height: 24, stroke: "currentColor" },
    external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(
...
function icon_zoom_out_IconZoomOut() {
  return external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(
    "svg",
    { width: 24, height: 24, stroke: "currentColor" },
    external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement("path", {
...
function toolbar_Toolbar(_ref) {
  ...
  return external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(
    ...
    external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(
      ...
      external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(icon_cursor_IconCursor, null)
    ),
    external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(
      toolbar_button,
      {
      ...
      external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(icon_pan_IconPan, null)
    ),
    external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(
      toolbar_button,
      {
      ...
      external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(icon_zoom_in_IconZoomIn, null)
    ),
    external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(
      toolbar_button,
      {
      ...
      external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(icon_zoom_out_IconZoomOut, null)
    ),
    external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(
      toolbar_button,
      {
      ...
      external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(icon_fit_IconFit, null)
    )
  );
}
...
function icon_arrow_IconArrow(_ref) {
  var open = _ref.open,
      position = _ref.position;
...
function miniature_toggle_button_defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }
...
function miniature_toggle_button_MiniatureToggleButton(_ref) {
  var _style;
  ...
  var style = (_style = {
    ...
    position: "absolute",
    bottom: 0
  }, miniature_toggle_button_defineProperty(_style, position === POSITION_LEFT ? 'left' : 'right', '0px'), miniature_toggle_button_defineProperty(_style, 'background', "rgba(19, 20, 22, 0.901961)"), miniature_toggle_button_defineProperty(_style, 'border', 0), miniature_toggle_button_defineProperty(_style, 'padding', 0), miniature_toggle_button_defineProperty(_style, 'outline', 0), miniature_toggle_button_defineProperty(_style, 'color', "#fff"), _style);
  ...
  return external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(
    ...
    external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(icon_arrow_IconArrow, { open: value.miniatureOpen, position: position })
  );
}

miniature_toggle_button_MiniatureToggleButton.propTypes = {
  value: external_root_PropTypes_commonjs2_prop_types_commonjs_prop_types_amd_prop_types_default.a.object.isRequired,
  onChangeValue: external_root_PropTypes_commonjs2_prop_types_commonjs_prop_types_amd_prop_types_default.a.func.isRequired,
  position: external_root_PropTypes_commonjs2_prop_types_commonjs_prop_types_amd_prop_types_default.a.oneOf([POSITION_RIGHT, POSITION_LEFT]).isRequired
};
...
function miniature_mask_MiniatureMask(_ref) {
...
miniature_mask_MiniatureMask.propTypes = {
...
/* harmony default export */ var miniature_mask = (RandomUID_RandomUID(miniature_mask_MiniatureMask));
...
var miniature_min = Math.min,
    miniature_max = Math.max;


function miniature_Miniature(props) {
  var _style;
  ...
  return external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(
    ...
    external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(miniature_toggle_button_MiniatureToggleButton, { value: value, onChangeValue: onChangeValue, position: position })
  );
}

miniature_Miniature.propTypes = {
  position: external_root_PropTypes_commonjs2_prop_types_commonjs_prop_types_amd_prop_types_default.a.oneOf([POSITION_RIGHT, POSITION_LEFT]).isRequired,
  value: external_root_PropTypes_commonjs2_prop_types_commonjs_prop_types_amd_prop_types_default.a.object.isRequired,
  onChangeValue: external_root_PropTypes_commonjs2_prop_types_commonjs_prop_types_amd_prop_types_default.a.func.isRequired,
...
var viewer_ReactSVGPanZoom = function (_React$Component) {
  ...
  function ReactSVGPanZoom(props, context) {
    ...
    _this.state = {
      value: value ? value : common_getDefaultValue(viewerWidth, viewerHeight, SVGWidth, SVGHeight, scaleFactorMin, scaleFactorMax),
  ...
  viewer_createClass(ReactSVGPanZoom, [{
    ...
    value: function componentWillReceiveProps(nextProps) {
      ...
      if (value.viewerWidth !== nextProps.width || value.viewerHeight !== nextProps.height) {
        nextValue = common_setViewerSize(nextValue, nextProps.width, nextProps.height);
        needUpdate = true;
      }
      ...
      if (value.SVGWidth !== SVGWidth || value.SVGHeight !== SVGHeight) {
        nextValue = common_setSVGSize(nextValue, SVGWidth, SVGHeight);
        needUpdate = true;
      }

      if (value.scaleFactorMin !== nextProps.scaleFactorMin || value.scaleFactorMax !== nextProps.scaleFactorMax) {
        nextValue = common_setZoomLevels(nextValue, nextProps.scaleFactorMin, nextProps.scaleFactorMax);
        needUpdate = true;
      }
    ...
    value: function autoPanLoop() {
      ...
      var nextValue = interactions_onInterval(null, this.ViewerDOM, this.getTool(), this.getValue(), this.props, coords);
        ...
        external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(
          'svg',
          {
            ...
            onMouseEnter: function onMouseEnter(event) {
              ...
              var nextValue = interactions_onMouseEnterOrLeave(event, _this2.ViewerDOM, _this2.getTool(), _this2.getValue(), _this2.props);
              if (_this2.getValue() !== nextValue) _this2.setValue(nextValue);
            },
          ...
          !(value.mode === MODE_ZOOMING) ? null : external_root_React_commonjs2_react_commonjs_react_amd_react_default.a.createElement(selection_Selection, { startX: value.startX, startY: value.startY, endX: value.endX, endY: value.endY })
        ),
...
viewer_ReactSVGPanZoom.defaultProps = {
  ...
  customMiniature: miniature_Miniature,
  disableZoomWithToolAuto: false,
  onZoom: null,
  onPan: null
};
// CONCATENATED MODULE: ./src/index.js
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "Viewer", function() { return src_Viewer; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "ReactSVGPanZoom", function() { return viewer; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "Toolbar", function() { return toolbar_Toolbar; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "Miniature", function() { return miniature_Miniature; });
...
var src_Viewer = function Viewer() {
